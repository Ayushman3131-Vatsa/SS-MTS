...
export const TaskManagementOverviewPage = () => {
  ...
  return <div className={styles.content}>
    ...
    <div className={styles.overviewGrid}>
      ...
      <section className={styles.panel}><div className={styles.panelHeader}><div><h2>Project summary</h2><p>Recently active projects</p></div><Button type="button" variant="ghost" className={styles.compactButton} onClick={() => navigate(appPath("/app/task-management/projects"))}>All projects</Button></div><div className={styles.projectSummary}>{projects.map((project) => <button key={project.project_id} type="button" onClick={() => navigate(appPath(`/app/task-management/projects/${project.project_id}/board`))}><span><strong>{project.project_key}</strong><small>{project.name}</small></span><StatusBadge value={project.status} /><ArrowRight size={14} /></button>)}</div></section>
    </div>
    <Overlay open={creatingProject} title="Create project" description="Set up a workspace and its first manager membership." onClose={() => setCreatingProject(false)} wide guardDirtyForm><ProjectForm onCancel={() => setCreatingProject(false)} onSaved={(project) => { setCreatingProject(false); navigate(appPath(`/app/task-management/projects/${project.project_id}/board`)); }} /></Overlay>
