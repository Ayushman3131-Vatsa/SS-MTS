...
import { canCreateTask, canExecuteTask, canManageProject, buildTaskManagementAccess } from "../../features/task-management/model/permissions";
...
const MembersView = ({ projectId, members, users, mayManage }: { projectId: string; members: ProjectMember[]; users: ReturnType<typeof useUsers>["data"] extends infer T ? NonNullable<T> : never; mayManage: boolean }) => {
  ...
  return <section className={styles.panel}><div className={styles.panelHeader}><div><h2>Project members</h2><p>Roles govern planning, collaboration and read-only access.</p></div></div>{mayManage && <form className={styles.memberForm} onSubmit={(event) => { event.preventDefault(); setError(null); void add.mutateAsync().then(() => setUserId("")).catch((cause) => setError(cause instanceof Error ? cause.message : "Could not add member.")); }}><Field label="User"><select value={userId} onChange={(event) => setUserId(event.target.value)} required><option value="">Select active user</option>{available.map((user) => <option key={user.user_id} value={user.user_id}>{user.name} · {user.role}</option>)}</select></Field><Field label="Project role"><select value={role} onChange={(event) => setRole(event.target.value as ProjectMemberRole)}>{MEMBER_ROLES.map((item) => <option key={item} value={item}>{item}</option>)}</select></Field><Button type="submit" className={styles.compactButton} disabled={!userId} loading={add.isPending}><UserPlus size={14} /> Add member</Button></form>}{error && <div className={styles.inlineAlert} role="alert">{error}</div>}
    <div className={styles.memberList}>{members.map((member) => { const user = userById.get(member.user_id); return <article key={member.membership_id}><UserAvatar user={user} size="medium" /><div><strong>{user?.name ?? "Unknown user"}</strong><span>{user?.email ?? member.user_id} · {user?.status ?? "Historical"}</span></div>{mayManage ? <select aria-label={`Role for ${user?.name ?? member.user_id}`} value={member.role} onChange={(event) => changeRole(member, event.target.value as ProjectMemberRole)}>{MEMBER_ROLES.map((item) => <option key={item}>{item}</option>)}</select> : <span className={styles.memberRole}>{member.role}</span>}{mayManage && <Button type="button" variant="ghost" className={styles.compactButton} onClick={() => setConfirm({ member })}>Remove</Button>}</article>; })}</div>
