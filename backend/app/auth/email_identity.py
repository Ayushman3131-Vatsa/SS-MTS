"""Global tenant-user email reservation helpers.

Tenant login is email-only, so account emails are globally unique. A new
tenant's primary contact reserves its future first-admin email before the
account exists. All reservation/account creation paths take the same
transaction-level advisory lock to close the cross-table race.
"""
...
async def reserve_new_tenant_contact(db: AsyncSession, email: str) -> str:
    ...
    if await get_user_by_email(db, normalized) is not None:
        raise ConflictError("This primary contact email is already used by a tenant account")
...
async def reserve_new_user_email(
    ...
    *,
    ...
    if contact_tenant is not None and not (
        allow_tenant_primary_contact and contact_tenant.tenant_id == tenant_id
    ):
